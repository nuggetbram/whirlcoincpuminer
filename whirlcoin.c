...
static void whirlpoolhash(void *state, const void *input)
{
    ...
    sph_whirlpool1 (&ctx_whirlpool, input, 80); //6
    sph_whirlpool1_close(&ctx_whirlpool, hashA); //7
...
int scanhash_whirlpool(int thr_id, uint32_t *pdata, const uint32_t *ptarget,
	uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	do {
		...
		whirlpoolhash(hash64, &endiandata);
